...
| `src/content/blog/` | MDX 博客文章（语言后缀命名） | ✓ |
...
| `scripts/` | 构建辅助脚本（OG / 图标校验 / redirects） | ✓ |
| `public/` | 静态资源 + 路由元文件 | ✓ |
...
1. **新工具清单**：完整步骤见 `CONTRIBUTING.md`「New Tool Checklist」。一句话总结：组件 + 4 语言路由 + `tools.ts` + `icons.ts` + 2 语言博客 + OG 验证 + tag 部署

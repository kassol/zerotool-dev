...
1. **新工具清单**：完整步骤见 `CONTRIBUTING.md`「New Tool Checklist」。一句话总结：组件 + 4 语言路由 + `tools.ts` + `icons.ts` + 4 语言 `content/tools/{slug}/` mdx + 2+ 语言博客 + OG 验证 + tag 部署。提交前跑 `node scripts/audit.mjs`
...
4. **博客命名**：非英语文章文件名必须 `{base-slug}-{lang}.mdx`（如 `csv-json-guide-zh.mdx`）。构建后 `generate-blog-redirects.mjs` 会自动追加 301
...
10. **工具 slug 重命名**：改 slug 必须 4 处同步——`src/data/tools.ts`、`src/data/icons.ts`、4 语言路由文件名、`public/_redirects` 加 4 条 301（en + zh + ja + ko）。删除工具时 `_redirects` 仍保留指向最相关替代品，避免 404

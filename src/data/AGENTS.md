...
interface ToolInfo {
  slug: string;                                    // URL slug，必须与 src/pages/tools/{slug}.astro 一致
  translations: Record<'en'|'zh'|'ja'|'ko', ToolTranslation>;  // 4 语言必填
  category: 'data'|'encoding'|'text'|'security'|'dev'|'api'|'color';
...
4. **category 枚举封闭**：要新增分类必须先改 `category` 联合类型，再同步 `src/pages/tools/index.astro` 的过滤器

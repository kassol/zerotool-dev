...
faqItems:
  ...
  - question: "有哪些限制？"
    answer: "最多 1000 张图，单张每边不超过 8192 px；生成的雪碧图每边不超过 16384 px，总像素不超过 16,777,216（例如 4096 × 4096）。按设计不旋转帧、不把大量图片拆成多页图集、只输出 PNG。需要旋转、多页图集或 Unity / Godot 专用图集格式时，请用 TexturePacker 或 free-tex-packer；需要 WebP 输出请用 TexturePacker。"
---

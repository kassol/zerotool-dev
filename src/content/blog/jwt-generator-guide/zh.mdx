...
zerotool.dev 的 [JWT Decoder](/tools/jwt-decoder) 只查看 header 和 payload，**不验证签名**。如需确认签名是否有效，请把 token 粘贴到 [jwt.io](https://jwt.io)，输入相同的 secret 和算法即可验签。
...
- [JWT Decoder →](/tools/jwt-decoder) — 解码和查看现有 JWT token

...
**[立即生成 JWT token →](/zh/tools/jwt-generator/)**
...
1. 打开 [JWT 生成器](/zh/tools/jwt-generator/)
...
zerotool.dev 的 [JWT Decoder](/zh/tools/jwt-decoder/) 只查看 header 和 payload，**不验证签名**。如需确认签名是否有效，请把 token 粘贴到 [jwt.io](https://jwt.io)，输入相同的 secret 和算法即可验签。
...
- [JWT Decoder →](/zh/tools/jwt-decoder/) — 解码和查看现有 JWT token
- [HMAC Generator →](/zh/tools/hmac-generator/) — 直接计算 HMAC 签名
- [Hash Generator →](/zh/tools/hash-generator/) — 生成 SHA-256 等哈希值

---

无需启动服务，秒级生成签名 token。**[打开 JWT 生成器 →](/zh/tools/jwt-generator/)**

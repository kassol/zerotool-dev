...
一行 JSON 没法读。第一件事永远是丢进 [JSON 格式化](/zh/tools/json-formatter/)。整理后：
...
- `created_at` 是 Unix 时间戳（1705332738）。预期之内，但顺手验一下。塞进[时间戳转换器](/zh/tools/timestamp-converter/)：**Jan 15, 2024, 14:32:18 UTC**。和日志条目对得上。OK。
...
丢进 [JWT 解码器](/zh/tools/jwt-decoder/)：
...
把实际 payload 配上这份 schema 跑一遍 [JSON Schema 验证器](/zh/tools/json-schema-validator/) 确认了猜测：`Additional properties are not allowed ('metadata' was unexpected)`。
...
丢进[文本对比](/zh/tools/diff-checker/)。diff 一目了然：`metadata` 是新加的。支付方直接加了字段，没升版本号、没通知集成方。（生产集成里这种事的发生频率，比你愿意承认的高。）
...
| 步骤 | 用到的工具 | 节省时间 |
|------|-----------|---------|
| 读 payload | [JSON 格式化](/zh/tools/json-formatter/) | 比手工解析省 ~8 分钟 |
| 验证时间戳 | [时间戳转换器](/zh/tools/timestamp-converter/) | 比写转换代码省 ~2 分钟 |
| 检查 auth token | [JWT 解码器](/zh/tools/jwt-decoder/) | 比 base64 + 手动解析省 ~5 分钟 |
| 用 schema 校验 payload | [JSON Schema 验证器](/zh/tools/json-schema-validator/) | 比写一段测试脚本省 ~10 分钟 |
| 确认 schema 变化 | [文本对比](/zh/tools/diff-checker/) | 比肉眼对比省 ~5 分钟 |

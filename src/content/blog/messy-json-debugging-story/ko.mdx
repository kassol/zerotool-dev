...
한 줄로는 읽을 수 없다. 첫 번째로 할 일은 언제나 같다 — [JSON 포매터](/ko/tools/json-formatter/)에 넣는다. 정리하면:
...
- `created_at`은 Unix 타임스탬프(1705332738)다. 예상대로지만 확인은 해 본다. [타임스탬프 변환기](/ko/tools/timestamp-converter/)에 붙여 넣는다: **Jan 15, 2024, 14:32:18 UTC**. 로그 시각과 일치. OK.
...
[JWT 디코더](/ko/tools/jwt-decoder/)에 붙여 넣는다:
...
실제 payload를 이 스키마와 함께 [JSON Schema 유효성 검사기](/ko/tools/json-schema-validator/)에 돌려 확인: `Additional properties are not allowed ('metadata' was unexpected)`.
...
[텍스트 차이 비교기](/ko/tools/diff-checker/)에 던진다. diff는 명백하다 — `metadata`는 신규 추가다. 결제 제공자가 버전 번호도 올리지 않고, 통합 측에 통보도 없이 필드를 추가했다. (프로덕션 통합에서 인정하기 싫을 만큼 자주 일어난다.)
...
| 단계 | 사용한 도구 | 절약 시간 |
|------|-----------|---------|
| payload 읽기 | [JSON 포매터](/ko/tools/json-formatter/) | 수동 파싱 대비 ~8분 |
| 타임스탬프 검증 | [타임스탬프 변환기](/ko/tools/timestamp-converter/) | 변환 코드 작성 대비 ~2분 |
| 인증 토큰 검사 | [JWT 디코더](/ko/tools/jwt-decoder/) | base64 디코딩+파싱 대비 ~5분 |
| 스키마 검증 | [JSON Schema 유효성 검사기](/ko/tools/json-schema-validator/) | 테스트 스크립트 작성 대비 ~10분 |
| 스키마 변경 확인 | [텍스트 차이 비교기](/ko/tools/diff-checker/) | 육안 비교 대비 ~5분 |

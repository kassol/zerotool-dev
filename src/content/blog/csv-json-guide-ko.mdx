...
[CSV를 JSON으로 즉시 변환하기 →](/tools/csv-json)
...
빠른 임시 변환을 위해 [ZeroTool의 CSV↔JSON 변환기](/tools/csv-json)는 브라우저에서 완전히 실행됩니다. CSV를 붙여넣거나 업로드하고, 옵션(구분자, 타입 파싱, 헤더 행)을 조정한 후 JSON을 다운로드하세요. 파일은 서버로 전송되지 않습니다.

**[CSV↔JSON 변환기 사용해보기 →](/tools/csv-json)**

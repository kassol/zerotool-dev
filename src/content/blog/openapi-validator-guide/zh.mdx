...
[在线验证 OpenAPI 规范 →](/tools/openapi-validator)
...
[ZeroTool OpenAPI 校验工具](/tools/openapi-validator) 完全在浏览器里运行，使用与服务端工具相同的验证逻辑。规范文件始终留在你本地，不上传，不注册。
...
校验工具支持两种格式。YAML 规范可以先用 [YAML Validator](/tools/yaml-validator) 检查语法，再做 OpenAPI 校验。JSON 规范可以先用 [JSON Formatter](/tools/json-formatter) 验证结构。
...
需要更深入的 Schema 验证，可以结合 [JSON Schema Validator](/tools/json-schema-validator) 使用。

## 现在就验证你的规范

[ZeroTool OpenAPI 校验工具](/tools/openapi-validator) 支持 YAML 和 JSON，覆盖 OpenAPI 3.0 和 3.1，显示带行号的错误位置，完全在浏览器内运行。

**[打开 OpenAPI 校验工具 →](/tools/openapi-validator)**

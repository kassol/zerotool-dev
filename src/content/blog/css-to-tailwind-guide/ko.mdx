...
[CSS를 Tailwind로 즉시 변환 →](/ko/tools/css-to-tailwind/)
...
`tailwind.config.js`를 `@theme` 블록으로 변환하고 더 이상 사용되지 않는 클래스를 이름 변경합니다. 이후 남은 커스텀 CSS에는 [CSS to Tailwind 변환기](/ko/tools/css-to-tailwind/)를 사용하세요.

## 온라인 CSS to Tailwind 변환기

일회성 변환에는 [ZeroTool의 CSS to Tailwind 변환기](/ko/tools/css-to-tailwind/)가 CSS 속성 블록을 실시간으로 Tailwind 등가물로 변환합니다. 왼쪽에 CSS를 붙여 넣으면 오른쪽에 Tailwind 클래스가 출력됩니다——빌드 단계, 설치 불필요.

**[CSS to Tailwind 변환기 사용해보기 →](/ko/tools/css-to-tailwind/)**

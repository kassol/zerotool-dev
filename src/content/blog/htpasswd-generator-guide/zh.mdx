...
搭建 Apache 或 Nginx 的 HTTP 基础认证需要一个 `.htpasswd` 文件——每行存储一个用户名与哈希密码的组合。正确生成这些哈希是第一道坎。本文介绍 htpasswd 的原理、算法选择、Apache/Nginx 配置方法，以及为什么我们的 [htpasswd 生成工具](/tools/htpasswd-generator) 在浏览器本地完成所有计算才是正确做法。
...
**第一步 — 生成 htpasswd 文件**，使用 [htpasswd 生成工具](/tools/htpasswd-generator) 或命令行：
...
**第一步 — 创建密码文件。** 使用 [浏览器生成工具](/tools/htpasswd-generator) 或系统已安装的 `htpasswd` 工具：
...
**[zerotool.dev 的 htpasswd 生成工具](/tools/htpasswd-generator) 完全在浏览器中运行。** bcrypt 哈希（以及其他所有算法实现）通过 JavaScript 在你的设备上本地执行。密码不会离开浏览器，没有服务器请求，没有输入字段日志记录。验证方法很简单：生成哈希时打开浏览器开发者工具的 Network 面板，你不会看到任何与密码相关的出站请求。

这使得它对生产环境凭据同样安全，在组织安全策略禁止向第三方网站粘贴密码的场景下也完全适用。

## 使用 htpasswd 生成工具

[ZeroTool 的 htpasswd 生成工具](/tools/htpasswd-generator) 支持：
...
**[立即使用 htpasswd 生成工具 →](/tools/htpasswd-generator)**

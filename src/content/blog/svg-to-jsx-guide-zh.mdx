...
从 Figma 导出 SVG 直接丢进 React 组件，看起来没问题——直到报错为止。`stroke-width` 不是合法的 JSX 属性，`class` 与 JavaScript 保留字冲突，`xmlns:xlink` 声明触发警告。手动转换 SVG 到 JSX 既繁琐又容易出错。本文梳理所有转换规则，最后介绍如何用 [SVG 转 JSX 工具](/tools/svg-to-jsx) 一步到位。
...
4. 复制导出的 SVG 代码。
5. 粘贴到 [SVG 转 JSX 工具](/tools/svg-to-jsx)，复制 React 组件输出。
...
[zerotool.dev 的 SVG 转 JSX 工具](/tools/svg-to-jsx) 完全在浏览器端运行，SVG 代码不会发送到任何服务器。这对包含未发布产品图标或专有设计系统素材的团队尤为重要——转换在本地完成，任何数据都不会离开你的设备。

## 在线 SVG 转 JSX 工具

[ZeroTool 的 SVG 转 JSX 工具](/tools/svg-to-jsx) 一次性处理上述所有规则：
...
如需转换 SVG 以外的 HTML 代码，请使用 [HTML 转 JSX 工具](/tools/html-to-jsx)。

**[立即使用 SVG 转 JSX 工具 →](/tools/svg-to-jsx)**

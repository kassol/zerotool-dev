  ...
  "mainEntity": [
    {
      ...
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Paste your YAML into ZeroTool's YAML to JSON converter at zerotool.dev/tools/yaml-json. The conversion happens instantly in your browser with no data sent to any server."
      }
    },
    {
      "@type": "Question",
      "name": "Is every valid JSON file also valid YAML?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Yes. YAML is a superset of JSON, so all valid JSON is valid YAML. However, YAML supports features that JSON does not, such as comments, anchors, and multi-line strings."
      }
    },
    {
      "@type": "Question",
      "name": "Why do YAML comments disappear when converting to JSON?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "JSON has no comment syntax. When converting YAML to JSON, all comment lines (starting with #) are stripped because there is no equivalent representation in JSON."
      }
    },
    {
      "@type": "Question",
      "name": "What happens to YAML anchors and aliases when converting to JSON?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "YAML anchors (&) and aliases (*) are expanded inline during conversion. The resulting JSON contains the full duplicated values rather than references, since JSON has no reference syntax."
      }
    },
    {
      "@type": "Question",
      "name": "How do I convert YAML to JSON from the command line?",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "Use the yq tool: `yq -o=json input.yaml`. Alternatively, use Python: `python3 -c \"import yaml,json,sys; print(json.dumps(yaml.safe_load(sys.stdin), indent=2))\" < input.yaml`"
      }
    }
  ]
}</script>

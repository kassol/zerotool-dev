...
HEIC 是 iPhone 自 2017 年起的默认容器。HEIC 用的是 ISOBMFF 结构而不是 JPEG marker，EXIF 的存放方式完全不一样。当前 ZeroTool 工具只针对 JPEG。要剥 HEIC 的 EXIF，本地用 `exiftool -all= file.heic`，或者先把 HEIC 转成 JPEG 再剥。

### TIFF、HEIC、PNG 文件

TIFF 用 IFD 树存 EXIF，结构和 JPEG 内的 EXIF 相似，但文件本身不是 JPEG marker 流。PNG 按 2017 年的扩展规范可以带 `eXIf` chunk。当前 ZeroTool 工具仅接受 JPEG —— 其他格式请本地用 `exiftool -all=` 或先转成 JPEG。

...
HEIC is the default container on iPhone since 2017. HEIC carries EXIF in a different (ISOBMFF) structure, not as a JPEG marker. The current ZeroTool tool focuses on JPEG. To strip HEIC EXIF, use `exiftool -all= file.heic` locally, or convert HEIC to JPEG first and then strip.

### TIFF, HEIC, PNG files

TIFF stores EXIF in IFD trees similar to the EXIF block inside a JPEG, but the file is not a JPEG marker stream. PNG can carry an `eXIf` chunk per the 2017 spec extension. The current ZeroTool tool accepts JPEG only — for other formats use `exiftool -all=` locally or convert to JPEG first.
...
| HEIC / TIFF | JPEG only (current) | Some | Yes | Yes |

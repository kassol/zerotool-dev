...
HEIC は 2017 年以降の iPhone の既定コンテナだ。HEIC は JPEG マーカーではなく ISOBMFF 構造で EXIF を持つ。現行 ZeroTool ツールは JPEG に絞っている。HEIC の EXIF を取り除くには、ローカルで `exiftool -all= file.heic` を使うか、HEIC を JPEG に変換してから除去する。

### TIFF / HEIC / PNG ファイル

TIFF は IFD ツリーで EXIF を持ち、JPEG 内部の EXIF と構造が似ているが、ファイルそのものは JPEG マーカーストリームではない。PNG は 2017 年の規格拡張により `eXIf` チャンクを持てる。現行 ZeroTool ツールは JPEG のみ受け付ける —— 他形式はローカルで `exiftool -all=` を使うか、JPEG に変換してから処理する。

...
<script type="application/ld+json">{
  ...
  "mainEntity": [
    {
      "@type": "Question",
      "name": "YAML 和 JSON 哪个更好？",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "取决于场景。YAML 适合人工编写的配置文件（可读性强、支持注释）；JSON 适合 API 数据交换（解析器支持更广、格式更严格）。两者各有擅长领域，开发中经常需要互转。"
      }
    },
    {
      "@type": "Question",
      "name": "YAML 转 JSON 后注释会保留吗？",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "不会。JSON 没有注释语法，YAML 中所有以 # 开头的注释在转换时都会被丢弃，无法恢复。"
      }
    },
    {
      "@type": "Question",
      "name": "为什么 YAML 中的 yes 变成了布尔值 true？",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "YAML 1.1 规范将 yes、no、on、off 定义为布尔值。大多数 YAML 库（如 PyYAML）默认遵循 YAML 1.1。如果需要字符串 yes，请加引号：\"yes\"。"
      }
    },
    {
      "@type": "Question",
      "name": "如何用命令行将 YAML 转换为 JSON？",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "推荐使用 yq 工具：`yq -o=json input.yaml`。也可以用 Python：`python3 -c \"import yaml,json,sys; print(json.dumps(yaml.safe_load(sys.stdin), indent=2, ensure_ascii=False))\" < input.yaml`"
      }
    },
    {
      "@type": "Question",
      "name": "YAML 锚点和引用转换成 JSON 后是什么样的？",
      "acceptedAnswer": {
        "@type": "Answer",
        "text": "YAML 的锚点（&）和引用（*）在转换为 JSON 时会被完全展开，引用的内容会在每个使用位置重复出现。JSON 没有引用或复用语法。"
      }
    }
  ]
}</script>

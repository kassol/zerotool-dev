...
[SQLをすぐに整形する →](/ja/tools/sql-formatter/)
...
何もインストールせずにアドホックな整形をするには、[ZeroToolのSQLフォーマッター](/ja/tools/sql-formatter/)がブラウザ内で処理します。任意のSQL（`SELECT`・`INSERT`・`CREATE TABLE`・ストアドプロシージャ・複雑なマルチJOINクエリ）を貼り付けるだけで整形結果が得られます。

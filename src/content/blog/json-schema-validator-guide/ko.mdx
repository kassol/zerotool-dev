...
[지금 JSON을 스키마로 검증하기 →](/tools/json-schema-validator)
...
스키마를 샘플 데이터에 대해 로컬로 테스트하려면 검증기 설정·파일 로드·코드 실행이 필요합니다. [ZeroTool JSON Schema 검증기](/tools/json-schema-validator)는 그 모든 번거로움을 없애줍니다:
...
**[JSON Schema 검증기 사용해보기 →](/tools/json-schema-validator)**

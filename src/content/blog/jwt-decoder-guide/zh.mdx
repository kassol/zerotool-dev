...
**[使用 ZeroTool JWT 解码器 →](/zh/tools/jwt-decoder/)**
...
1. 用 [ZeroTool JWT 解码器](/zh/tools/jwt-decoder/) 解码 Token，检查 `exp`（是否过期）、`iss`（颁发者是否正确）、`aud`（受众是否匹配）
...
**[在线解码任意 JWT，浏览器本地运算 →](/zh/tools/jwt-decoder/)**
